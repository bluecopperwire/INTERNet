...
describe('Phase 3 Student Attendance workflow', () => {
  ...
  it('contains the exact summary/history labels, filters, columns, routes, and no legacy Late UI', () => {
    ...
    for (const column of ["header: 'Date'", "header: 'Time'", "header: 'Rendered'", "header: 'Status'"]) expect(sharedHistory).toContain(column)
    expect(sharedHistory).toContain('DataTable')
