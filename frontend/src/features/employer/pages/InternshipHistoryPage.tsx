...
export function EmployerInternshipHistoryPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      ...
      <div className={styles.tableCard}><div className={styles.tableScroller}><table className={styles.table}><thead><tr>{INTERNSHIP_HISTORY_COLUMNS.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.internshipAssignmentId}><td><strong>{row.studentFullName}</strong></td><td>{row.jobTitle}</td><td>{row.strandProgram || 'N/A'}</td><td><span className={styles.statusPill}>{assignmentStatusLabel(row.assignmentStatus)}</span></td><td><button type="button" className={styles.viewButton} onClick={() => navigate(`/employer/internship-history/${row.internshipAssignmentId}`)}><Eye size={16} />View</button></td></tr>)}</tbody></table></div>{loading && <p className={styles.emptyState}>Loading internship history...</p>}{error && <p className={styles.emptyState} role="alert">{error}</p>}{!loading && !error && rows.length === 0 && <div className={styles.emptyState}><strong>No internship history yet</strong><br />There are currently no internship records to display.</div>}</div>
