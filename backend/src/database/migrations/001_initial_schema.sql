...
CREATE TYPE public.personnel_verification_status_enum AS ENUM ('pending', 'approved', 'rejected');
...
CREATE TABLE public.local_authentication_credential (
  user_account_id integer NOT NULL,
  password_hash text NOT NULL,
  password_changed_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT pk_local_authentication_credential PRIMARY KEY (user_account_id),
  CONSTRAINT ck_local_auth_credential_password_hash_not_blank CHECK (btrim(password_hash) <> '')
);

CREATE TABLE public.external_authentication_identity (
  ...
  created_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT pk_external_authentication_identity PRIMARY KEY (external_authentication_identity_id),
...
CREATE TABLE public.authentication_session (
...
CREATE TABLE public.registration_onboarding (
...
CREATE TABLE public.peso_personnel (
  ...
  photo_file_path text,
  verification_status public.personnel_verification_status_enum NOT NULL DEFAULT 'pending',
  ...
  CONSTRAINT ck_peso_personnel_verification_remark_not_blank CHECK (verification_remark IS NULL OR btrim(verification_remark) <> ''),
  CONSTRAINT ck_peso_personnel_verification_review_consistency CHECK (
    (verification_status = 'pending' AND reviewed_at IS NULL AND reviewed_by_user_account_id IS NULL AND verification_remark IS NULL)
    OR (verification_status IN ('approved', 'rejected') AND reviewed_at IS NOT NULL AND reviewed_by_user_account_id IS NOT NULL)
  )
);
...
CREATE TABLE public.peso_personnel_verification_history (
  peso_personnel_verification_history_id integer GENERATED ALWAYS AS IDENTITY,
  peso_personnel_id integer NOT NULL,
  previous_verification_status public.personnel_verification_status_enum NOT NULL,
  new_verification_status public.personnel_verification_status_enum NOT NULL,
  changed_by_user_account_id integer NOT NULL,
  changed_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT pk_peso_personnel_verification_history PRIMARY KEY (peso_personnel_verification_history_id),
  CONSTRAINT ck_peso_verification_history_status_changed CHECK (previous_verification_status <> new_verification_status)
);

ALTER TABLE public.local_authentication_credential ADD CONSTRAINT fk_local_auth_credential_user_account FOREIGN KEY (user_account_id) REFERENCES public.user_account(user_account_id) ON DELETE CASCADE;
ALTER TABLE public.external_authentication_identity ADD CONSTRAINT fk_external_auth_identity_user_account FOREIGN KEY (user_account_id) REFERENCES public.user_account(user_account_id) ON DELETE CASCADE;
ALTER TABLE public.authentication_session ADD CONSTRAINT fk_authentication_session_user_account FOREIGN KEY (user_account_id) REFERENCES public.user_account(user_account_id) ON DELETE CASCADE;
...
ALTER TABLE public.peso_personnel ADD CONSTRAINT fk_peso_personnel_reviewed_by FOREIGN KEY (reviewed_by_user_account_id) REFERENCES public.user_account(user_account_id) ON DELETE RESTRICT;
...
ALTER TABLE public.peso_personnel_verification_history ADD CONSTRAINT fk_peso_verification_history_personnel FOREIGN KEY (peso_personnel_id) REFERENCES public.peso_personnel(peso_personnel_id) ON DELETE CASCADE;
ALTER TABLE public.peso_personnel_verification_history ADD CONSTRAINT fk_peso_verification_history_changed_by FOREIGN KEY (changed_by_user_account_id) REFERENCES public.user_account(user_account_id) ON DELETE RESTRICT;
...
CREATE INDEX ix_external_auth_identity_user_account ON public.external_authentication_identity (user_account_id);
CREATE INDEX ix_authentication_session_user_account ON public.authentication_session (user_account_id);
CREATE INDEX ix_authentication_session_account_revoked ON public.authentication_session (user_account_id, revoked_at);
CREATE INDEX ix_authentication_session_expires_at ON public.authentication_session (expires_at);
CREATE INDEX ix_registration_onboarding_expires_at ON public.registration_onboarding (expires_at);
CREATE INDEX ix_registration_onboarding_consumed_at ON public.registration_onboarding (consumed_at);
...
CREATE INDEX ix_peso_personnel_verification_status ON public.peso_personnel (verification_status);
CREATE INDEX ix_peso_personnel_reviewed_by ON public.peso_personnel (reviewed_by_user_account_id);
...
CREATE INDEX ix_peso_verification_history_personnel_time ON public.peso_personnel_verification_history (peso_personnel_id, changed_at);
CREATE INDEX ix_peso_verification_history_changed_by ON public.peso_personnel_verification_history (changed_by_user_account_id);
...
CREATE FUNCTION public.fn_check_authentication_method_integrity()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  FROM public.local_authentication_credential
  WHERE user_account_id = account_id;
  ...
  IF local_count + external_count = 0 THEN
    RAISE EXCEPTION 'Account % must retain at least one authentication method', account_id;
  END IF;

  IF external_count > 0 AND account_role <> 'student' THEN
    RAISE EXCEPTION 'External authentication identities are permitted only for student accounts';
  END IF;
...
CREATE FUNCTION public.fn_revoke_account_sessions()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.account_status IS DISTINCT FROM OLD.account_status
    AND NEW.account_status IN ('suspended', 'archived') THEN
...
CREATE FUNCTION public.fn_validate_peso_verification()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  actor_id integer;
  personnel_account_id integer;
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.verification_status <> 'pending' THEN
      RAISE EXCEPTION 'A new QC PESO registration must begin with pending verification';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF NEW.verification_status IS NOT DISTINCT FROM OLD.verification_status THEN
    IF NEW.reviewed_at IS DISTINCT FROM OLD.reviewed_at
      OR NEW.reviewed_by_user_account_id IS DISTINCT FROM OLD.reviewed_by_user_account_id
      OR NEW.verification_remark IS DISTINCT FROM OLD.verification_remark THEN
      RAISE EXCEPTION 'Verification review metadata may change only during a verification transition';
    END IF;
    RETURN NEW;
  END IF;

  IF NOT (
    (OLD.verification_status = 'pending' AND NEW.verification_status IN ('approved', 'rejected'))
    OR (OLD.verification_status = 'rejected' AND NEW.verification_status = 'pending')
  ) THEN
    RAISE EXCEPTION 'Invalid QC PESO verification transition: % -> %', OLD.verification_status, NEW.verification_status;
  END IF;

  actor_id := public.fn_current_status_actor();
  IF actor_id IS NULL THEN
    RAISE EXCEPTION 'QC PESO verification transitions require an authenticated actor';
  END IF;

  IF OLD.verification_status = 'pending' THEN
    IF NEW.reviewed_by_user_account_id IS DISTINCT FROM actor_id
      OR NEW.reviewed_at IS NULL THEN
      RAISE EXCEPTION 'QC PESO review metadata must identify the authenticated admin actor';
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM public.user_account
      WHERE user_account_id = actor_id
        AND user_role = 'admin'
        AND account_status = 'active'
        AND deleted_at IS NULL
    ) THEN
      RAISE EXCEPTION 'Only an active admin may approve or reject QC PESO verification';
    END IF;
  ELSE
    SELECT user_account_id INTO personnel_account_id
    FROM public.peso_personnel
    WHERE peso_personnel_id = NEW.peso_personnel_id;
    IF actor_id IS DISTINCT FROM personnel_account_id THEN
      RAISE EXCEPTION 'Rejected QC PESO verification may be resubmitted only by its account owner';
    END IF;
    IF NEW.reviewed_at IS NOT NULL
      OR NEW.reviewed_by_user_account_id IS NOT NULL
      OR NEW.verification_remark IS NOT NULL THEN
      RAISE EXCEPTION 'QC PESO resubmission must clear current review metadata';
    END IF;
  END IF;
...
BEGIN
  CASE TG_TABLE_NAME
    ...
    WHEN 'internship_assignment' THEN
      ...
      END IF;
    WHEN 'peso_personnel' THEN
      IF NEW.verification_status IS DISTINCT FROM OLD.verification_status THEN
        IF actor_id IS NULL THEN
          RAISE EXCEPTION 'QC PESO verification history requires an authenticated actor';
        END IF;
        INSERT INTO public.peso_personnel_verification_history (
          peso_personnel_id,
          previous_verification_status,
          new_verification_status,
          changed_by_user_account_id
        ) VALUES (
          NEW.peso_personnel_id,
...
CREATE TRIGGER trg_peso_personnel_verification BEFORE INSERT OR UPDATE ON public.peso_personnel FOR EACH ROW EXECUTE FUNCTION public.fn_validate_peso_verification();
...
CREATE TRIGGER trg_peso_verification_history AFTER UPDATE OF verification_status ON public.peso_personnel FOR EACH ROW EXECUTE FUNCTION public.fn_record_status_history();
...
CREATE TRIGGER trg_peso_verification_history_append_only BEFORE UPDATE OR DELETE ON public.peso_personnel_verification_history FOR EACH ROW EXECUTE FUNCTION public.fn_block_status_history_mutation();
