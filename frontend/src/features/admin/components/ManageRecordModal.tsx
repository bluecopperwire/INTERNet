...
export function ManageRecordModal({ recordId, recordRole, onClose }: ManageRecordModalProps) {
  ...
  const handleArrayChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
    ...
    setEditForm(prev => {
      return { ...prev, [fieldName]: value.split(',').map((s: string) => s.trim()) }
    })
  }
  ...
  const renderEditableText = (label: string, name: string, value: string, placeholder?: string, span?: boolean) => {
    return (
  ...
  const renderStudentSummary = (data: StudentRecord) => (
    <div className={styles.summarySection}>
      ...
      <div className={styles.infoGrid}>
        {renderEditableText('First Name (Extracted from Full Name)', 'fullName', data.fullName)}
        ...
        <div className={styles.infoGroup}>
          ...
          {isEditing ? (
             <select 
               ...
               value={(editForm as any).flexibleAssignment ? 'true' : 'false'}
        ...
        <div className={styles.infoGroup} style={{ gridColumn: 'span 2' }}>
          ...
          {isEditing ? (
            <input 
              type="text" 
              className={styles.editInput} 
              value={((editForm as any).preferredIndustries || []).join(', ')} 
        ...
        <div className={styles.infoGroup} style={{ gridColumn: 'span 2' }}>
          ...
          {isEditing ? (
            <input 
              type="text" 
              className={styles.editInput} 
              value={((editForm as any).scheduleAvailability || []).join(', ')} 
        ...
        <div className={styles.modalContainer}>
          ...
          <div className={styles.modalBody}>
            ...
            <div className={styles.commonProfileCard}>
              <div className={styles.commonProfileHeader}>
                ...
                <div className={styles.badgeRow}>
                  ...
                  <span className={`${styles.statusBadge} ${getStatusBadgeClass(record.status)}`}>{record.status}</span>
                </div>
              </div>
            ...
            <div className={styles.actionsToolbar}>
              <button className={styles.toggleEditBtn} onClick={() => {
                if (isEditing) {
