...
function AttendancePage() {
  ...
  const { today, monthData, isLoading, isCheckingIn, error, checkIn } = useAttendance(visibleMonth.getFullYear(), visibleMonth.getMonth())
              ...
              <div className={styles.todayDetails}>
                ...
                <div className={styles.detailRow}>
                  ...
                  <div>
                    <h3>Working Days</h3>
                    <p>
                      {today.workingDays} | {today.shiftStart} - {today.shiftEnd}
                    </p>
                  </div>
                </div>
              </div>
      ...
      {!isLoading && !today && !error && (
        <div className={styles.emptyState}>
          <section className={styles.todayPanel}>
            <h2>No active internship yet</h2>
            <p>There is currently no active internship to track.</p>
          </section>
          <section className={styles.summaryPanel}>
            <h2>Monthly Summary</h2>
            <p>Attendance information will appear once an internship assignment is available.</p>
          </section>
        </div>
      )}
    </>
...
function AttendanceCalendar({ monthData, onPrevious, onNext }: { monthData: AttendanceMonth; onPrevious: () => void; onNext: () => void }) {
  ...
  const monthLabel = new Intl.DateTimeFormat('en-PH', {
    month: 'long',
    year: 'numeric',
  }).format(new Date(monthData.year, monthData.month, 1))
    ...
    <section className={styles.calendarPanel} aria-labelledby="calendar-heading">
      ...
      <div className={styles.calendarGrid} role="grid" aria-label={monthLabel}>
        {WEEKDAYS.map((weekday) => (
          <span className={styles.weekday} role="columnheader" key={weekday}>
            {weekday}
          </span>
        ))}
        {calendarDays.map((day) =>
          day.day === null ? (
            ...
            <span
              className={`${styles.calendarDay} ${day.status ? styles[day.status] : ''}`}
              role="gridcell"
              aria-label={`${monthLabel} ${day.day}${day.status ? `, ${day.status}` : ''}`}
              key={day.key}
            >
              {day.day}
            </span>
...
function MonthlySummary({ monthData }: { monthData: AttendanceMonth }) {
  const summaryItems = [
    {
      label: 'Days Present',
      value: String(monthData.summary.daysPresent).padStart(2, '0'),
      className: styles.presentCard,
    },
    {
      label: 'Absences',
      value: String(monthData.summary.absences).padStart(2, '0'),
      className: styles.absentCard,
    },
    {
      label: 'Late Arrivals',
      value: String(monthData.summary.lateArrivals).padStart(2, '0'),
      className: styles.lateCard,
    },
    {
      label: 'Attendance Rate',
      value: `${monthData.summary.attendanceRate}%`,
    ...
    <section className={styles.summaryPanel} aria-labelledby="summary-heading">
      ...
      <div className={styles.summaryGrid}>
        {summaryItems.map((item) => (
          <article className={`${styles.summaryCard} ${item.className}`} key={item.label}>
...
function buildCalendarDays(monthData: AttendanceMonth) {
  ...
  return [
    ...
    ...Array.from({ length: numberOfDays }, (_, index) => ({
      key: `day-${index + 1}`,
      day: index + 1,
      status: statusByDay.get(index + 1),
    })),
  ]
}
