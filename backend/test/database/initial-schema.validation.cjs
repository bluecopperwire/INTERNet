...
async function main() {
  ...
  try {
    ...
    const inventory = await client.query(`
      SELECT
        (SELECT count(*) FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace
         WHERE n.nspname='public' AND t.typtype='e')::integer AS enum_count,
        (SELECT count(*) FROM information_schema.tables
         WHERE table_schema='public' AND table_type='BASE TABLE' AND table_name <> 'migrations')::integer AS table_count,
        (SELECT count(*)
         FROM information_schema.columns c
         JOIN information_schema.tables t
           ON t.table_schema=c.table_schema AND t.table_name=c.table_name
         WHERE c.table_schema='public' AND t.table_type='BASE TABLE'
           AND c.table_name <> 'migrations')::integer AS column_count,
        (SELECT count(*) FROM pg_constraint c JOIN pg_namespace n ON n.oid=c.connamespace
         WHERE n.nspname='public' AND c.contype='f')::integer AS foreign_key_count,
        (SELECT count(*) FROM information_schema.views
         WHERE table_schema='public')::integer AS view_count
    `);
    assert.deepEqual(inventory.rows[0], {
      enum_count: 18,
      table_count: 26,
      column_count: 244,
      foreign_key_count: 33,
      view_count: 7,
    });
    pass('DBML v3 structural inventory matches 18 enums, 26 tables, 244 columns, 33 foreign keys, and 7 views');

    const identityMismatch = await client.query(`
      SELECT table_name, column_name
      FROM information_schema.columns
      WHERE table_schema='public'
        AND column_name = table_name || '_id'
        AND table_name NOT IN ('local_authentication_credential', 'student_preferred_industry')
        AND identity_generation IS DISTINCT FROM 'ALWAYS'
    `);
    assert.equal(identityMismatch.rowCount, 0);
    pass('all generated primary identifiers use GENERATED ALWAYS AS IDENTITY');

    const secretViewColumns = await client.query(`
      SELECT table_name, column_name
      FROM information_schema.columns
      WHERE table_schema='public'
        AND table_name LIKE 'vw\\_%' ESCAPE '\\'
        AND column_name IN ('password_hash', 'refresh_token_hash', 'onboarding_token_hash', 'provider_subject')
    `);
    assert.equal(secretViewColumns.rowCount, 0);
    pass('views expose no authentication secrets or external subject identifiers');
    ...
    await client.query(
      `INSERT INTO public.external_authentication_identity (
       ...
       ) VALUES ($1, 'google', 'google-subject-alpha', 'student1@example.test')`,
      [studentAccount1],
    );
    const combinedMethods = await client.query(
      `SELECT
         EXISTS (SELECT 1 FROM public.local_authentication_credential WHERE user_account_id=$1) AS has_local,
         EXISTS (SELECT 1 FROM public.external_authentication_identity WHERE user_account_id=$1) AS has_google`,
      [studentAccount1],
    );
    assert.deepEqual(combinedMethods.rows[0], {
      has_local: true,
      has_google: true,
    });
    pass('student account supports both local and Google authentication');

    await expectReject(
      client,
      `INSERT INTO public.external_authentication_identity (
         user_account_id, authentication_provider, provider_subject, provider_email
       ) VALUES ($1, 'google', 'google-subject-alpha', 'student2@example.test')`,
      [studentAccount2],
      /uq_external_auth_identity_provider_subject/,
      'Google provider-subject identity is globally unique',
    );
    ...
    await client.query(
      `INSERT INTO public.authentication_session (
         ...
         ($1, '11111111-1111-4111-8111-111111111111', 'session-hash-one', CURRENT_TIMESTAMP + INTERVAL '7 days'),
         ($1, '22222222-2222-4222-8222-222222222222', 'session-hash-two', CURRENT_TIMESTAMP + INTERVAL '7 days')`,
      [studentAccount2],
    );
    const activeSessions = await client.query(
      `SELECT count(*)::integer AS count FROM public.authentication_session
       WHERE user_account_id=$1 AND revoked_at IS NULL`,
      [studentAccount2],
    );
    assert.equal(activeSessions.rows[0].count, 2);
    pass('multiple concurrent authentication session families are supported');
    await client.query(
      `UPDATE public.authentication_session
       SET refresh_token_hash='session-hash-one-rotated', last_used_at=CURRENT_TIMESTAMP
       WHERE token_family_id='11111111-1111-4111-8111-111111111111'`,
    );
    const sessionRotation = await client.query(
      `SELECT token_family_id::text, refresh_token_hash
       FROM public.authentication_session WHERE user_account_id=$1 ORDER BY token_family_id`,
      [studentAccount2],
    );
    assert.deepEqual(sessionRotation.rows, [
      {
        token_family_id: '11111111-1111-4111-8111-111111111111',
        refresh_token_hash: 'session-hash-one-rotated',
      },
      {
        token_family_id: '22222222-2222-4222-8222-222222222222',
        refresh_token_hash: 'session-hash-two',
      },
    ]);
    pass('refresh rotation updates only the selected session family');

    const onboarding = (
      await client.query(
        `INSERT INTO public.registration_onboarding (
           onboarding_token_hash, authentication_provider, provider_subject,
           verified_email, intended_user_role, expires_at
         ) VALUES ('onboarding-hash-one', 'google', 'onboarding-subject',
           'onboarding@example.test', 'student', CURRENT_TIMESTAMP + INTERVAL '30 minutes')
         RETURNING registration_onboarding_id`,
      )
    ).rows[0].registration_onboarding_id;
    await client.query(
      `UPDATE public.registration_onboarding
       SET onboarding_token_hash='onboarding-hash-reset', expires_at=CURRENT_TIMESTAMP + INTERVAL '30 minutes'
       WHERE registration_onboarding_id=$1`,
      [onboarding],
    );
    await expectReject(
      client,
      `INSERT INTO public.registration_onboarding (
         onboarding_token_hash, provider_subject, verified_email, intended_user_role, expires_at
       ) VALUES ('onboarding-hash-two', 'onboarding-subject', 'other@example.test', 'student', CURRENT_TIMESTAMP + INTERVAL '30 minutes')`,
      [],
      /uq_registration_onboarding_provider_subject/,
      'Google onboarding provider-subject row is reused rather than duplicated',
    );
    await expectReject(
      client,
      `INSERT INTO public.registration_onboarding (
         onboarding_token_hash, provider_subject, verified_email, intended_user_role, expires_at
       ) VALUES ('onboarding-hash-role', 'onboarding-role', 'role@example.test', 'company', CURRENT_TIMESTAMP + INTERVAL '30 minutes')`,
      [],
      /ck_registration_onboarding_student_role_only/,
      'Google onboarding is student-only',
    );
    await client.query(
      `UPDATE public.registration_onboarding SET consumed_at=CURRENT_TIMESTAMP
       WHERE registration_onboarding_id=$1`,
      [onboarding],
    );
    pass('onboarding supports hash rotation and consumption metadata');
    ...
    const rejectedPersonnelAccount = await insertAccount(
      client,
      'rejected-personnel@example.test',
      'peso_personnel',
    );
    const rejectedPersonnel = (
      await client.query(
        `INSERT INTO public.peso_personnel (
           user_account_id, first_name, last_name, sex, birth_date,
           address_line, address_barangay, address_district, address_city,
           contact_number, contact_email, employee_id, position, department,
           employee_id_file_path
         ) VALUES ($1, 'Riley', 'Reviewer', 'unspecified', DATE '1991-01-01',
           '4 Test Street', 'Test Barangay', 'Test District', 'Quezon City',
           '09173333333', 'rejected-personnel@example.test', 'VAL-002', 'Officer',
           'Placement', '/validation/id-2.png')
         RETURNING peso_personnel_id`,
        [rejectedPersonnelAccount],
      )
    ).rows[0].peso_personnel_id;
    ...
    await client.query(
      `UPDATE public.peso_personnel
       SET verification_status='approved', reviewed_at=CURRENT_TIMESTAMP,
           reviewed_by_user_account_id=$1
       WHERE peso_personnel_id=$2`,
      [actorAccount, personnel],
    );
    await client.query(
      `UPDATE public.peso_personnel
       SET verification_status='rejected', reviewed_at=CURRENT_TIMESTAMP,
           reviewed_by_user_account_id=$1, verification_remark='Replace employee ID'
       WHERE peso_personnel_id=$2`,
      [actorAccount, rejectedPersonnel],
    );
    await expectReject(
      client,
      `UPDATE public.peso_personnel
       SET verification_remark='Changed without transition'
       WHERE peso_personnel_id=$1`,
      [rejectedPersonnel],
      /metadata may change only during a verification transition/,
      'rejected verification metadata cannot be edited outside a transition',
    );
    await client.query(
      "SELECT set_config('app.current_user_account_id', $1, true)",
      [String(rejectedPersonnelAccount)],
    );
    await client.query(
      `UPDATE public.peso_personnel
       SET verification_status='pending', reviewed_at=NULL,
           reviewed_by_user_account_id=NULL, verification_remark=NULL,
           employee_id_file_path='/validation/id-2-replaced.png'
       WHERE peso_personnel_id=$1`,
      [rejectedPersonnel],
    );
    const verificationHistory = await client.query(
      `SELECT previous_verification_status, new_verification_status, changed_by_user_account_id
       FROM public.peso_personnel_verification_history
       WHERE peso_personnel_id=$1 ORDER BY peso_personnel_verification_history_id`,
      [rejectedPersonnel],
    );
    assert.deepEqual(verificationHistory.rows, [
      {
        previous_verification_status: 'pending',
        new_verification_status: 'rejected',
        changed_by_user_account_id: actorAccount,
      },
      {
        previous_verification_status: 'rejected',
        new_verification_status: 'pending',
        changed_by_user_account_id: rejectedPersonnelAccount,
      },
    ]);
    pass('QC PESO rejection and owner resubmission record append-only actor history');
    const verificationHistoryId = (
      await client.query(
        `SELECT peso_personnel_verification_history_id
         FROM public.peso_personnel_verification_history
         WHERE peso_personnel_id=$1 LIMIT 1`,
        [rejectedPersonnel],
      )
    ).rows[0].peso_personnel_verification_history_id;
    await expectReject(
      client,
      `DELETE FROM public.peso_personnel_verification_history
       WHERE peso_personnel_verification_history_id=$1`,
      [verificationHistoryId],
      /append-only/,
      'QC PESO verification history rejects deletes',
    );
    ...
    await expectReject(
      ...
      `UPDATE public.peso_personnel
       SET verification_status='approved', reviewed_at=CURRENT_TIMESTAMP,
           reviewed_by_user_account_id=$1
       WHERE peso_personnel_id=$2`,
      [rejectedPersonnelAccount, rejectedPersonnel],
      /Only an active admin/,
      'only an active admin may review QC PESO verification',
    );
    await client.query(
      "SELECT set_config('app.current_user_account_id', $1, true)",
      [String(actorAccount)],
    ...
    await expectReject(
      client,
      `INSERT INTO public.user_account (email, user_role)
       VALUES ('ACTOR@example.test', 'admin')`,
      [],
    ...
    const revokedSessions = await client.query(
      `SELECT count(*)::integer AS count FROM public.authentication_session
       WHERE user_account_id=$1 AND revoked_at IS NOT NULL`,
      [studentAccount2],
    );
    assert.equal(revokedSessions.rows[0].count, 2);
    pass('suspension or archive revokes all sessions and restoration does not revive them');
  ...
  {
    const transaction = new Client(connection);
    ...
    try {
      ...
      const googleOnlyAccount = (
        await transaction.query(
          `INSERT INTO public.user_account (email, user_role)
           VALUES ('google-only@example.test', 'student') RETURNING user_account_id`,
        )
      ).rows[0].user_account_id;
      await transaction.query(
        `INSERT INTO public.external_authentication_identity (
           user_account_id, provider_subject, provider_email
         ) VALUES ($1, 'google-only-subject', 'google-only@example.test')`,
        [googleOnlyAccount],
      );
      await insertStudent(transaction, googleOnlyAccount, 'GoogleOnly');
      await transaction.query('COMMIT');
      pass('student account can commit with a Google identity only');
    } finally {
  ...
  await expectTransactionReject(
    async (transaction) => {
      await transaction.query(
        `INSERT INTO public.user_account (email, user_role)
         VALUES ('no-auth-method@example.test', 'admin')`,
      );
    },
    /must retain at least one authentication method/,
    'account cannot commit without an authentication method',
  );
  ...
  await expectTransactionReject(
    async (transaction) => {
      const accountId = (
        await transaction.query(
          `INSERT INTO public.user_account (email, user_role)
           VALUES ('admin-google@example.test', 'admin') RETURNING user_account_id`,
        )
      ).rows[0].user_account_id;
      await transaction.query(
        `INSERT INTO public.external_authentication_identity (
           user_account_id, provider_subject, provider_email
         ) VALUES ($1, 'admin-google-subject', 'admin-google@example.test')`,
        [accountId],
      );
    },
    /permitted only for student accounts/,
    'company, QC PESO, and admin roles cannot use Google identity',
  );
  ...
  await expectTransactionReject(
    async (transaction) => {
      const accountId = await insertAccount(
        transaction,
        'remove-only-method@example.test',
        'admin',
      );
      await transaction.query(
        'DELETE FROM public.local_authentication_credential WHERE user_account_id=$1',
        [accountId],
      );
    },
    /must retain at least one authentication method/,
  ...
  await expectTransactionReject(
    async (transaction) => {
      await transaction.query(
        `WITH account AS (
           INSERT INTO public.user_account (email, user_role)
           VALUES ('missing-profile@example.test', 'student')
           RETURNING user_account_id
         )
         INSERT INTO public.local_authentication_credential (user_account_id, password_hash)
         SELECT user_account_id, 'validation-bcrypt-hash' FROM account`,
      );
    },
