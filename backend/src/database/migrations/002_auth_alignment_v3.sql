...
-- A previously generated private 001 variant used normalized credential/session
-- tables that are not part of approved DBML V3. It was observed in the local
-- development volume with no application rows. Realign that exact empty variant
-- safely, but refuse to discard any populated legacy authentication state.
DO $$
DECLARE
...
CREATE TABLE public.oauth_identity (
  ...
  CONSTRAINT ck_oauth_identity_provider_subject_not_blank
    CHECK (btrim(provider_subject) <> ''),
  CONSTRAINT ck_oauth_identity_provider_email_not_blank
...
CREATE TABLE public.auth_session (
  ...
  CONSTRAINT ck_auth_session_refresh_token_hash_not_blank
    CHECK (btrim(refresh_token_hash) <> ''),
  CONSTRAINT ck_auth_session_expires_after_creation
    CHECK (expires_at > created_at),
  CONSTRAINT ck_auth_session_revoked_after_creation
    CHECK (revoked_at IS NULL OR revoked_at >= created_at)
);

ALTER TABLE public.oauth_identity
  ADD CONSTRAINT fk_oauth_identity_user_account
  FOREIGN KEY (user_account_id)
  REFERENCES public.user_account(user_account_id)
  ON DELETE CASCADE;

ALTER TABLE public.auth_session
  ADD CONSTRAINT fk_auth_session_user_account
  FOREIGN KEY (user_account_id)
  REFERENCES public.user_account(user_account_id)
  ON DELETE CASCADE;

CREATE UNIQUE INDEX uq_oauth_identity_provider_subject
  ON public.oauth_identity (authentication_provider, provider_subject);
CREATE UNIQUE INDEX uq_oauth_identity_account_provider
  ON public.oauth_identity (user_account_id, authentication_provider);
CREATE INDEX ix_auth_session_user_account
  ON public.auth_session (user_account_id);
CREATE INDEX ix_auth_session_expires_at
  ON public.auth_session (expires_at);
CREATE UNIQUE INDEX uq_auth_session_one_active_per_account
  ON public.auth_session (user_account_id)
  WHERE revoked_at IS NULL;

CREATE FUNCTION public.fn_validate_oauth_identity_account()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF account_role IS DISTINCT FROM 'student'::public.user_role_enum THEN
    RAISE EXCEPTION 'OAuth identities are permitted only for student accounts';
  END IF;
  RETURN NEW;
END;
$$;

CREATE FUNCTION public.fn_validate_auth_session_account()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF NEW.revoked_at IS NULL AND status IS DISTINCT FROM 'active'::public.account_status_enum THEN
    RAISE EXCEPTION 'Active authentication sessions require an active account';
  END IF;
  RETURN NEW;
END;
$$;

CREATE FUNCTION public.fn_check_account_auth_method_integrity()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  SELECT user_role, password_hash
  INTO account_role, stored_password_hash
  FROM public.user_account
  ...
  IF account_role IN ('company', 'peso_personnel', 'admin')
     AND stored_password_hash IS NULL THEN
    RAISE EXCEPTION 'Account role % requires a password hash', account_role;
  END IF;

  IF account_role = 'student'
     AND stored_password_hash IS NULL
     AND NOT EXISTS (
       SELECT 1 FROM public.oauth_identity
       WHERE user_account_id = account_id
     ) THEN
    RAISE EXCEPTION 'Student account % requires a password hash or OAuth identity', account_id;
  END IF;
...
CREATE TRIGGER trg_oauth_identity_updated_at
  BEFORE UPDATE ON public.oauth_identity
  FOR EACH ROW EXECUTE FUNCTION public.fn_set_updated_at();
CREATE TRIGGER trg_auth_session_updated_at
  BEFORE UPDATE ON public.auth_session
  FOR EACH ROW EXECUTE FUNCTION public.fn_set_updated_at();
CREATE TRIGGER trg_oauth_identity_student_only
  BEFORE INSERT OR UPDATE OF user_account_id ON public.oauth_identity
  FOR EACH ROW EXECUTE FUNCTION public.fn_validate_oauth_identity_account();
CREATE TRIGGER trg_auth_session_active_account
  BEFORE INSERT OR UPDATE OF user_account_id, revoked_at ON public.auth_session
  FOR EACH ROW EXECUTE FUNCTION public.fn_validate_auth_session_account();

CREATE CONSTRAINT TRIGGER trg_user_account_auth_method_integrity
  AFTER INSERT OR UPDATE OF password_hash, user_role ON public.user_account
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE FUNCTION public.fn_check_account_auth_method_integrity();
CREATE CONSTRAINT TRIGGER trg_oauth_identity_auth_method_integrity
  AFTER INSERT OR UPDATE OF user_account_id OR DELETE ON public.oauth_identity
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE FUNCTION public.fn_check_account_auth_method_integrity();
