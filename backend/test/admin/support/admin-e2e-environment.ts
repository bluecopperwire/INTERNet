...
export class AdminE2eEnvironment {
  ...
  async start(): Promise<void> {
    this.databaseName = `admin_e2e_${randomUUID().replaceAll('-', '')}`;
    this.container = await new PostgreSqlContainer('postgres:16-alpine')
      .withDatabase(this.databaseName)
      .withUsername('admin_e2e')
    ...
    process.env.JWT_ACCESS_SECRET = 'admin-e2e-access-secret';
    process.env.JWT_REFRESH_SECRET = 'admin-e2e-refresh-secret';
    process.env.FILE_STORAGE_DRIVER = 'test';
    process.env.LOCAL_PUBLIC_STORAGE_ROOT = join(this.testCwd, 'uploads');

    // ConfigModule must load only after the disposable container variables exist.
  ...
  assertDisposableDatabase(): void {
    if (process.env.NODE_ENV !== 'test' || !this.container) {
      throw new Error(
        'Refusing Admin E2E database access outside a live test container.',
      );
    }
    ...
    if (
      ...
      !this.databaseName.startsWith('admin_e2e_')
    ) {
      throw new Error(
        'Refusing destructive reset: database is not the Admin E2E container.',
      );
    }
  }
