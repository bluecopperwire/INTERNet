...
function AttendancePage() {
  ...
  return <div className={styles.attendanceLayout}><div className={styles.primaryColumn}><section className={styles.todayPanel} aria-labelledby="today-heading"><header className={styles.panelHeader}><h2 id="today-heading">Today&apos;s Status</h2><div className={styles.statusGroup}><div className={styles.statusItem}><span className={styles.statusLabel}>Assignment Status</span><span className={styles.statusBadge}>{assignmentLabels[assignment.assignmentStatus as Exclude<AssignmentStatus, 'finalized'>]}</span></div><div className={styles.statusItem}><span className={styles.statusLabel}>Attendance Status</span><span className={styles.statusBadge}>{attendanceTag}</span></div></div></header><div className={styles.todayDetails}><TimeField title="Clock In Time" value={data.today?.timeIn} empty="No clock in time yet." /><TimeField title="Clock Out Time" value={data.today?.timeOut} empty="No clock out time yet." /></div><div className={styles.clockActions}><button className={styles.checkInButton} type="button" disabled={!canClockIn || action !== null} onClick={() => void runClockAction('in')}>{action === 'in' ? 'Clocking In...' : 'Clock In'}</button><button className={styles.checkOutButton} type="button" disabled={!canClockOut || action !== null} onClick={() => void runClockAction('out')}>{action === 'out' ? 'Clocking Out...' : 'Clock Out'}</button></div></section><Summary summary={data.summary} onHistory={() => navigate(`/intern-seeker/attendance-history/${assignment.internshipAssignmentId}`, { state: { attendanceHistoryBackPath: '/intern-seeker/attendance' } })} /></div><aside className={styles.secondaryColumn}><AttendanceCalendar data={data} month={visibleMonth} onPrevious={() => setVisibleMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))} onNext={() => setVisibleMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))} /></aside></div>
}
