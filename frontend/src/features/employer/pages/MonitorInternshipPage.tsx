  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      <div className={styles.summaryGrid}>
        ...
        <SummaryCard label="Completed Interns" value={internships.filter((item) => ['Complete (Company)', 'Complete (Student)', 'Finalized'].includes(item.status)).length} />
      ...
      <div className={styles.toolbar}>
        ...
        <label className={styles.statusFilter}><SlidersHorizontal size={16} /><span className={styles.srOnly}>Filter internship status</span><select value={status} onChange={(event) => { setStatus(event.target.value); resetPage() }}><option value="All">All Statuses</option><option value="Pending">Pending</option><option value="On Going">On Going</option><option value="Complete (Company)">Complete (Company)</option><option value="Complete (Student)">Complete (Student)</option><option value="Awaiting Completion">Awaiting Completion</option><option value="Withdrawn by Student">Withdrawn by Student</option><option value="Cancelled">Cancelled</option><option value="Finalized">Finalized</option></select></label>
      </div>

      <div className={styles.tableCard}>
        <div className={styles.tableScroller}>
          <table className={styles.table}><thead><tr><th>Student Name</th><th>Job Title</th><th>Remaining Hours</th><th>Status</th><th>Action</th></tr></thead><tbody>{displayed.map((internship) => <tr key={internship.applicantId}><td><strong>{internship.studentName}</strong></td><td>{internship.jobTitle}</td><td>{Math.max(internship.requiredHours - internship.renderedHours, 0)} hrs</td><td><span className={styles.statusPill}>{internship.status}</span></td><td><div className={styles.rowActions}><button type="button" className={styles.viewButton} onClick={() => navigate(`/employer/manage-internship/${internship.applicantId}`)}><Eye size={16} />View</button>{internship.status === 'Finalized' && <button type="button" className={styles.viewButton} onClick={() => setDeleteTarget(internship)}><Trash2 size={16} />Delete</button>}</div></td></tr>)}</tbody></table>
        </div>
