...
export function EmployerInternshipHistoryDetailsPage() {
  ...
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    <section className={styles.statusCard}><header className={styles.statusHeader}><h2>History Actions</h2><p>Attendance remains available while this record is visible to the Company.</p></header><footer className={styles.statusActions}><button type="button" className={styles.completeButton} onClick={() => navigate(`/employer/attendance/${assignmentId}`)}><Clock3 size={16} /> View Attendance History</button>{details.status.canDelete && <button type="button" className={styles.deleteRecordButton} onClick={() => setShowDelete(true)}><Trash2 size={16} /> Delete</button>}</footer></section>
