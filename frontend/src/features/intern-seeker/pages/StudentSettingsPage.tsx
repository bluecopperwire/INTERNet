...
export function StudentSettingsPage() {
  ...
  const updatePassword = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error, 'Failed to update password. Please check your current password and try again.')
        ...
        <section className={styles.settingsSection}>
          ...
          <form className={styles.passwordForm} onSubmit={updatePassword}>
            <div className={styles.passwordFields}>
              <label className={styles.fullWidth}><span>Current Password</span><input type="password" value={currentPassword} placeholder="Enter current password" autoComplete="current-password" onChange={(event) => { setCurrentPassword(event.target.value); setMessage('') }} required /></label>
            ...
            <footer className={styles.sectionFooter}>
              <button className={styles.passwordButton} type="submit" disabled={isSubmitting}><LockKeyhole size={17} aria-hidden="true" /><span>{isSubmitting ? 'Updating...' : 'Update Password'}</span></button>
            </footer>
          </form>
        </section>
