...
const formatAddress = (profile: QCPesoProfile) => [profile.addressLine, profile.barangay, profile.district && `District ${profile.district}`, profile.city].filter(Boolean).join(', ')
...
export function QCPesoProfilePage() {
  ...
  useEffect(() => {
    qcpesoService.getProfile().then(setProfile).finally(() => setIsLoading(false))
  }, [])
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      const updatedProfile = { ...profile, avatarUrl: String(reader.result) }
  ...
  if (isLoading || !profile) return <main className={styles.pageContainer}><div className={styles.loading}>Loading QC PESO profile...</div></main>
      ...
      <section className={styles.headerSection}>
        <div className={styles.headerContainer}>
          <div className={styles.profileMainInfo}>
            <button type="button" className={styles.avatarButton} onClick={() => fileInputRef.current?.click()} aria-label="Upload profile picture">
            ...
            <div className={styles.profileMeta}>
              ...
              <span className={styles.contactLine}><MapPin size={16} />{formatAddress(profile)}</span>
              <span className={styles.contactLine}><Mail size={16} />{profile.email}</span>
            </div>
          </div>
          <button type="button" className={styles.editButton} onClick={() => navigate('/qcpeso/profile/edit')}>Edit Profile</button>
        </div>
      </section>
        ...
        <div className={styles.profileGrid}>
          <ProfileSection icon={<UserRound size={22} />} title="Personal Information">
            <DetailsList>
              ...
              <DetailItem label="Birthdate" value={new Date(`${profile.birthdate}T00:00:00`).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
          ...
          <ProfileSection icon={<Building2 size={22} />} title="Work Information" fullWidth>
            <DetailsList>
              <DetailItem label="Employee ID" value={profile.employeeIdNumber} />
...
function ProfileSection({ children, icon, title, fullWidth = false }: { children: React.ReactNode; icon: React.ReactNode; title: string; fullWidth?: boolean }) {
  return <section className={`${styles.card} ${fullWidth ? styles.fullWidthCard : ''}`}><header className={styles.cardHeader}><span className={styles.iconCircle}>{icon}</span><h2>{title}</h2></header>{children}</section>
}
