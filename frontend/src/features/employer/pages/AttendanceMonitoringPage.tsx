...
export function AttendanceMonitoringPage() {
  ...
  return <main className={styles.pageContainer}>
    ...
    <section className={styles.mainContent}>
      ...
      <div className={styles.tableCard}><div className={styles.tableScroller}><table className={styles.table}><thead><tr>{ATTENDANCE_MONITOR_COLUMNS.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.internshipAssignmentId}><td><strong>{row.studentFullName}</strong></td><td>{row.jobTitle}</td><td>{row.strandProgram || 'N/A'}</td><td><span className={`${styles.statusPill} ${styles[row.status] ?? ''}`}>{attendanceStatusLabel(row.status)}</span></td><td><button type="button" className={styles.actionBtn} onClick={() => navigate(`/employer/attendance/${row.internshipAssignmentId}`, { state: { attendanceHistoryBackPath: '/employer/attendance' } })}><Eye size={14} />View</button></td></tr>)}</tbody></table></div>{loading && <p className={styles.noData}>Loading attendance...</p>}{error && <p className={styles.noData} role="alert">{error}</p>}{!loading && !error && rows.length === 0 && <p className={styles.noData}>No interns match the selected date and status.</p>}</div>
